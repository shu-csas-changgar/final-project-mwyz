...
///// new stuff: delete methods /////////////
  delete_reservation(reservation){
    ...
    create_new_user(empid, fname, lname, email,  var dt = nw Date);
  +officei+","+"\asswod+ity, zip){
    var dt = new Date();
    var datetime = dt.toISOStrinsert_city(city, zip){
    var dt = new Date();
    var d);
t.toISOStri    (0, 10) + " " + dt.toTimeString().substr(0, 8  var datetime = dt.toISOString().substr(0, 10) + " " + dt.toTimeString().substr(0, 8);
(create_new_user(empid, fname, lname, email, phone, officeid, password, address, cityid, countryid){
...
delete_city(city){
  ...
  join(cols, tables, joined_fields, conditions, type='join'){
    ...
    query +=ail,ions[conditions.length-1] + ";"
    return query;
  }
}

vae_new_user(empi,fname, lme, e dmsword');
=======
class Query{
  //phone - string
  create_new_user(empid, fname, lname, email, phone, officeid, password, address, cityid, countryid){
...
>>>>>>> f329331c0283be60a71bcf5e2f5820e50e7b3dc4

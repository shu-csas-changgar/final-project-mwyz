class Query{
 ...
 //city, zip
  ...
  contains_city(dct){
    return "select cityid from abc.city where name = " + "\'"+ dct["city"] + "\'"+ ";";
  }

  contains_office(dct){
    return "select officeid from abc.office where address = " + dct["address"] + ";";
  }

  contains_country(dct){
    return "select countryid from abc.country where name = " + "\'"+ dct["country"] + "\'"+ ";";
  }

  contains_deviceType(dct){
    return "select devicetypeid from abc.devicetypes where devicetype = " + "\'"+ dct["devicetype"] + "\'"+ ";";
  }
...
// employeeid, deviceType, deviceTypeid
  ...
  add_existing_device(dct){
    var dt = new Date();
    var datetime = dt.toISOString().substr(0, 10) + " " + dt.toTimeString().substr(0, 8);
    return "insert into abc.devices (deviceid, devicetypeid, leased, employeeid, updated) values("+"\'"+dct["deviceid"] +"\'"+dct["deviceTypeid"]+"\'"+"," + "\'"+ dct["leased"] + "\'"+  ","
  + "\'"+ dct["empid"] + "\'"+ ","+"\'"+datetime+"\'"+"); ";
  }
...
// address, cityid, countryid
  add_new_office(dct){
    var dt = new Date();
    var datetime = dt.toISOString().substr(0, 10) + " " + dt.toTimeString().substr(0, 8);
    return "insert into abc.office (address, cityid, countryid, updated) values("+"\'"+dct["address"]+"\'"+","+"\'"+dct["cityid"]+"\'"+","+"\'"+dct["countryid"]+"\'"+
    ",\'"+datetime+"\'"+");";
  }
...
console.log(Q.insert_reservation({"vendor": "sdf", "stock": "500","deviceType": "laptop", "deviceid": "325", "officeid": "34", "deviceTypeid": "laptop", "empid": "32", "issue" : "werwer", "components": "sfd" ,"price": "35435"}));
